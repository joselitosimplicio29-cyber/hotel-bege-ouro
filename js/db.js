...
function _mapReservation(r) {
  ...
  return {
    ...
    valorRestante:   Number(r.valor_restante),
...
const DB = {
  ...
  /* ===== Quartos — leitura síncrona do cache ===== */
  ...
  async saveRoom(room) {
    ...
    if (room.id) {
      await _sb.from('rooms').update(payload).eq('id', room.id);
      const idx = _cache.rooms.findIndex(r => r.id === room.id);
      if (idx >= 0) _cache.rooms[idx] = { ..._cache.rooms[idx], ...payload };
    } else {
      payload.id = 'q' + Date.now().toString(36);
      const { data } = await _sb.from('rooms').insert(payload).select().single();
  ...
  async deleteRoom(id) {
    await _sb.from('rooms').delete().eq('id', id);
  ...
  /* ===== Clientes — leitura síncrona do cache ===== */
  ...
  async saveClient(c) {
    ...
    if (c.id) {
      await _sb.from('clients').update(payload).eq('id', c.id);
      const idx = _cache.clients.findIndex(x => x.id === c.id);
      if (idx >= 0) _cache.clients[idx] = { ..._cache.clients[idx], ...c };
    } else {
      const { data } = await _sb.from('clients').insert(payload).select().single();
      const mapped = _mapClient(data);
      _cache.clients.push(mapped);
      return mapped;
    }
    return c;
  },
  ...
  async findOrCreateClient({ nome, cpf, telefone, email }) {
    ...
    if (c) return c;
    /* Verifica no banco (pode ter sido inserido por outro dispositivo) */
    let query = _sb.from('clients').select('*');
    if (cpf)   query = query.eq('cpf', cpf);
    else if (email) query = query.eq('email', email);
    const { data: existing } = await query.maybeSingle();
    if (existing) {
      c = _mapClient(existing);
  ...
  /* ===== Reservas — leitura síncrona do cache ===== */
  reservations() { return _cache.reservations; },
  reservation(id) { return _cache.reservations.find(r => r.id === id); },
  reservationByCode(code) { return _cache.reservations.find(r => r.codigo === code); },
  ...
  async saveReservation(res) {
    const payload = {
      ...
      quarto_id:       res.quartoId,
      entrada:         res.entrada,
      saida:           res.saida,
      diarias:         res.diarias,
      hospedes:        res.hospedes,
      valor_diaria:    res.valorDiaria,
      valor_total:     res.valorTotal,
      valor_pago:      res.valorPago || 0,
      valor_restante:  res.valorRestante,
      forma_pagamento: res.formaPagamento,
      status_pagamento: res.statusPagamento,
      status_reserva:  res.statusReserva,
      origem:          res.origem,
      observacoes:     res.observacoes || '',
    };
    if (res.id) {
      await _sb.from('reservations').update(payload).eq('id', res.id);
      ...
      const { data } = await _sb.from('reservations').insert(payload).select().single();
      const mapped = _mapReservation(data);
      _cache.reservations.unshift(mapped);
      await this.refreshRoomStatuses();
      return mapped;
    }
  },
  ...
  async checkIn(id) {
    ...
    const q = _cache.rooms.find(x => x.id === r.quartoId);
  ...
  async refreshRoomStatuses() {
    ...
    for (const q of _cache.rooms) {
      ...
      const futura = _cache.reservations.find(r =>
        r.quartoId === q.id &&
        ['confirmada','pendente'].includes(r.statusReserva) &&
        r.entrada > today
      );
      const newStatus = ativa
        ? (ativa.statusReserva === 'em_hospedagem' ? 'ocupado' : 'reservado')
        : futura ? 'reservado' : 'disponivel';
      if (q.status !== newStatus) {
        q.status = newStatus;
        await _sb.from('rooms').update({ status: newStatus, updated_at: new Date().toISOString() }).eq('id', q.id);
      }
    }
  },
  ...
  async addConsumption(c) {
    const payload = {
      ...
      produto:       c.produto,
      qtd:           c.qtd,
      valor_unit:    c.valorUnit,
      valor_total:   c.valorTotal,
      funcionario_id: c.funcionarioId || null,
      data_hora:     c.dataHora || new Date().toISOString(),
    };
    const { data } = await _sb.from('consumptions').insert(payload).select().single();
    const mapped = _mapConsumption(data);
    _cache.consumptions.unshift(mapped);
    return mapped;
  },

  async removeConsumption(id) {
    await _sb.from('consumptions').delete().eq('id', id);
    _cache.consumptions = _cache.consumptions.filter(c => c.id !== id);
  },

  /* ===== Pagamentos ===== */
  payments(reservaId = null) {
    const all = _cache.payments;
  ...
  async addPayment(p) {
    ...
    const mapped = _mapPayment(data);
    _cache.payments.unshift(mapped);

    /* Atualiza valor_pago na reserva */
